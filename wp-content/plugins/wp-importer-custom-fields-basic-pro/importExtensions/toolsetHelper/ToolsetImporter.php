...
class ToolsetImporter {
	...
	public function separateElements($parentGroupName,$postTypeValue) {
		...
		{
			$result = $wpdb->get_results("SELECT id FROM ".$wpdb->prefix."posts WHERE post_title='{$explodedParent[$i]}' and post_type='{$postTypeValue}'");
	...
	public function insertRelationship($groupRelationId,$postId,$childPostId)
	{
		...
		if($plugin_version < '3.4.1'){
			...
			$postId = $childPostId;
			$metaKey = 'toolset-post-sortorder';
			$meta_value = '1';

			update_post_meta($postId, $metaKey, $meta_value);
		}
		else{
