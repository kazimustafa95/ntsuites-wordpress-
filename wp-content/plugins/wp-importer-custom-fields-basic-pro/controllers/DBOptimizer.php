...
class DBOptimizer {
	...
	public function DbOptimizer() {
		...
		if(sanitize_text_field($_POST['orphaned']) == 'true') {
			$array_post_id = '';
			$get_post_id = $wpdb->get_results("select DISTINCT pm.post_id from $wpdb->postmeta pm JOIN $wpdb->posts wp on wp.ID = pm.post_id");
			foreach($get_post_id as $postID) {
				$array_post_id .= $postID->post_id . ',';
			}
			$array_post_id = substr($array_post_id, 0, -1);
			$wpdb->get_results("DELETE FROM $wpdb->postmeta where post_id not in ($array_post_id)",ARRAY_A);
