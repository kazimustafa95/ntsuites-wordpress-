...
class WordpressCustomImport {
    ...
    function set_wordpress_custom_values($header_array ,$value_array , $map, $post_id , $type,$group_name, $hash_key,$gmode,$templatekey){	
        $post_values = [];
        ...
        if($group_name == 'CORECUSTFIELDS'){
			...
			$this->wordpress_custom_user_import_function($post_values, $post_id ,$type , 'off',$header_array,$value_array, $hash_key,$gmode,$templatekey);	
		}
		else{
    ...
    public function wordpress_custom_user_import_function ($data_array, $pID, $importType , $core_serialize_info,$header_array,$value_array, $hash_key,$gmode,$templatekey) {
                    ...
                    elseif($custom_key == '_opening_time'){
                        ...
                        foreach($urls as $images){
                            ...
                            if(empty($results=='')){
                               ...
                               $results[] = $media_instance->image_meta_table_entry('', $pID, 'image_gallery_ids', $images, $hash_key, 'wordpress_custom', 'post',$templatekey,$gmode);                               
                            }
                        }
