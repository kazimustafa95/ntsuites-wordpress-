  ...
  ;(function($, window, document, undefined) {
    ...
    */
    Owl.prototype.registerEventHandlers = function() {
      ...
      //handle enable animation on drag / swipe option
      if(this.settings.enableAnimationOnDragSwipe == true){

        //turn off drag event
        this.settings.mouseDrag = false;

        //call custom event handler on mouse down here
        this.$stage.on('mousedown.owl.core', $.proxy(this.onAnimatedDragStart, this));

        //copied from default behaviour
        this.$stage.on('dragstart.owl.core selectstart.owl.core', function() { return false });

        //do same on touch devices
        this.$stage.on('touchstart.owl.core', $.proxy(this.onAnimatedDragStart, this));


      }
      
      if (this.settings.mouseDrag && this.settings.enableAnimationOnDragSwipe == false) {
        this.$element.addClass(this.options.dragClass);
        this.$stage.on('mousedown.owl.core', $.proxy(this.onDragStart, this));
        this.$stage.on('dragstart.owl.core selectstart.owl.core', function() { return false });
      }
      
      if (this.settings.touchDrag && this.settings.enableAnimationOnDragSwipe == false){
    ...
    /**
    * Handles `touchstart` and `mousedown` events with enabled animation.
    * @param {Event} event - The event arguments.
    */
    Owl.prototype.onAnimatedDragStart = function(event) {
            
      var startX = event.type === 'mousedown' ? event.clientX : event.originalEvent.touches[0].clientX;;
      var startY = event.type === 'mousedown' ? event.clientY : event.originalEvent.touches[0].clientY;;
      ...
      $(document).on("mousemove.owl.core touchmove.owl.core", function(event) {

        if (isMouseDown == true) {

          var deltaX = (event.type === 'mousemove' ? event.clientX : event.originalEvent.touches[0].clientX) - startX;
          var deltaY = (event.type === 'mousemove' ? event.clientY : event.originalEvent.touches[0].clientY) - startY;
      
          direction = getDirection(deltaX, deltaY);

          startX = event.type === 'mousemove' ? event.clientX : event.originalEvent.touches[0].clientX;
          startY = event.type === 'mousemove' ? event.clientY : event.originalEvent.touches[0].clientY;
        
        }
      });      

      $(document).on('mouseup.owl.core touchend.owl.core', function() {
                        ...
                        Navigation.prototype.initialize = function() {
                          ...
                          : $('<div>').addClass(settings.navContainerClass).appendTo(this.$element));
                            ...
                            if(settings.changeItemOnClick){
                              ...
                              var carousel = this;           
                              var clonedItems = carousel._core.clones().length / 2;
                              
                              setTimeout(function(){
                                var owlItems = carousel._core.$stage.children();
                                
                                owlItems.on('click', $.proxy(function(e){
                                  
                                  var index = $(e.currentTarget).index();
                                  carousel.to(index - clonedItems, settings.navSpeed, true);
