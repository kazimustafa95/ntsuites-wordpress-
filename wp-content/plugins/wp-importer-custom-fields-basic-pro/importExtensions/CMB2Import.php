...
class CMB2Import {
    ...
    function set_cmb2_values($header_array ,$value_array , $map, $post_id , $type,$hash_key,$gmode,$templatekey){	
		$post_values = [];
		$helpers_instance = ImportHelpers::getInstance();	
		$post_values = $helpers_instance->get_header_values($map , $header_array , $value_array);
		
		$this->cmb2_import_function($post_values, $post_id,$hash_key,$gmode,$templatekey);

    }
    public function cmb2_import_function ($data_array,$pID,$hash_key,$gmode,$templatekey) {
		$createdFields = array();
		$media_instance = MediaHandling::getInstance();
		$get_csvpro_settings = get_option('sm_uci_pro_settings');
		$prefix = isset($get_csvpro_settings['cmb2']) ? $get_csvpro_settings : '';

		foreach ($data_array as $dkey => $dvalue) {
			$createdFields[] = $dkey;
			if($dkey == $prefix.'image'){				
				$darray[$prefix.'image_id'] = $media_instance->image_meta_table_entry('', $pID, $image_key , $dvalue, $hash_key, 'cmb2',$templatekey,$gmode);
			...
			elseif($dkey == $prefix.'file_list'){
				...
				foreach($exploded_file_items as $ekey => $evalue){
					$imageid =  $media_instance->media_handling($evalue, $pID);
				...
				foreach($exploded_group_items as $gkey => $gvalue){
					...
					$rep_group[$gkey]['image_id'] = $media_instance->media_handling($exploded_line_items[2], $pID);
