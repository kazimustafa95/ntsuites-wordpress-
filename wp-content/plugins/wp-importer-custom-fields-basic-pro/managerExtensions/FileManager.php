...
class FileManager {
    ...
    public function manage_records($hash_key ,$selected_type , $file_name , $total_rows){
        ...
        $get_name = $wpdb->get_results( "SELECT original_file_name FROM ultimate_cfimporter_pro_smackuci_events " );
        ...
        if(!empty($get_name)){
            ...
            if(in_array($file_name , $inserted_name_values)){
                $get_revision = $wpdb->get_results( "SELECT revision FROM ultimate_cfimporter_pro_smackuci_events WHERE original_file_name = '$file_name' " );
        ...
        $get_data =  $wpdb->get_results("SELECT skipped , created , updated FROM $log_table_name WHERE hash_key = '$hash_key' ");
			...
			$getid = $wpdb->get_results("SELECT distinct( id ) from ultimate_cfimporter_pro_smackuci_events where import_type = '$import_type' and eventKey = '$hash_key'",ARRAY_A);		
		if(!empty($getid)){
			$wpdb->update('ultimate_cfimporter_pro_smackuci_events', array(
    ...
    public function downloadFile()
	{
		...
		$get_event_key = $wpdb->get_results($wpdb->prepare("select original_file_name,name, filepath, filetype , eventKey from ultimate_cfimporter_pro_smackuci_events where revision = %d and original_file_name = %s", $revision , $filename));
    ...
    public function downloadAllFiles(){
        ...
        $upload_dir = $upload['baseurl'];
        $upload_url = $upload_dir . '/smack_uci_uploads/imports/';
		$Details = $wpdb->get_results($wpdb->prepare("select name,revision from ultimate_cfimporter_pro_smackuci_events where original_file_name = %s",$filename));
		$eventkeys = $wpdb->get_col($wpdb->prepare("select eventkey from ultimate_cfimporter_pro_smackuci_events where original_file_name = %s",$filename));
		if(isset($eventkeys)){
		...
		if(is_array($pathList)) :
            ...
            $zipped_file = $this->create_zip($newfile , $downloadableFilePath, $zip_path , false);
	 ...
	 * @return boolean
	 */
    public function create_zip($newfile, $files_list = array(), $file_path = '', $overwrite = false){
		$valid_files = array();
    ...
    public function deleteFiles()
	{
		check_ajax_referer('smack-importer-custom-fields-basic-pro', 'securekey');
        global $wpdb;
        $version = sanitize_text_field($_POST['revision']);
		$filename = sanitize_text_field($_POST['filename']);
		$file_extension = pathinfo($filename, PATHINFO_EXTENSION);
		if(empty($file_extension)){
			$file_extension = 'xml';
        ...
        $upload_path = FileManager::$smack_csv_instance->create_upload_dir();
        $response = [];
		$filekey = $wpdb->get_results($wpdb->prepare("select eventKey,id from ultimate_cfimporter_pro_smackuci_events where revision=%d and original_file_name=%s", $version,$filename));
		...
		$revision = '';
		if (!empty($version)) {
			$template_id = $wpdb->get_col($wpdb->prepare("select id from ultimate_cfimporter_pro_mappingtemplate where eventKey = %s", $hash_key));
			if (!empty($template_id)) {
				$schedule_detail = $wpdb->get_results($wpdb->prepare("select id,frequency from ultimate_cfimporter_pro_scheduled_import where templateid = %d", $template_id[0]));
				if (!empty($schedule_detail) && $schedule_detail[0]->frequency != 0) {
					...
					$wpdb->delete("ultimate_cfimporter_pro_scheduled_import", array('templateid' => $template_id[0]));
				}
			}
			if(file_exists($file_path)){
    ...
    public function deleteEventDetails($file_path , $id) {
		...
		if(file_exists($file_path)):
			...
			$wpdb->delete("ultimate_cfimporter_pro_smackuci_events", array('id' => $id));
    ...
    public function delete_all_files() {
        ...
        $upload_path = FileManager::$smack_csv_instance->create_upload_dir();
		$importas = $module;
		$get_eventkey = $wpdb->get_col($wpdb->prepare("select eventkey from ultimate_cfimporter_pro_smackuci_events where original_file_name = %s",$filename));	
		$get_id = $wpdb->get_col($wpdb->prepare("select id from ultimate_cfimporter_pro_smackuci_events where original_file_name = %s",$filename));		
		...
		$mod_array = Array('Posts'=>'post','Pages'=>'page');
		...
		if(!empty($records[0]) && !empty($module)):
			...
			if(!empty($file_path)) {
				...
				for($i = 0 ; $i<$count_id ; $i++){
					$ids = $get_id[$i];
					$wpdb->delete("ultimate_cfimporter_pro_smackuci_events", array('id' => $ids));
				}
    ...
    public function delete_wp_records($records,$module,$importas) {
		...
		if(is_plugin_active('jet-engine/jet-engine.php')){
			...
			foreach($get_slug_name as $key=>$get_slug){
				$value=$get_slug->slug;
				$optionaltype=$value;						
				if($optionaltype == $module){
					$table_name = 'jet_cct_'.$module;
					foreach($records as $key => $recordId){	
						$wpdb->delete($wpdb->$table_name, array('_ID' => $recordId), array('%d'));
					}					
				}
			}
		}
	...
	public function display_events(){
		...
		$value = [];
		$fileInfo = [];
		$distinctEvents = $wpdb->get_results("select distinct(original_file_name) From ultimate_cfimporter_pro_smackuci_events order by id desc");
		...
		if(empty($distinctEvents)){
			...
			foreach($distinctEvents as $key => $eventData) {
				...
				$eventsInformation = $wpdb->get_results("select * from ultimate_cfimporter_pro_smackuci_events where original_file_name = '{$csvName}' order by id desc");	
				...
				$event_id = array();
                    foreach ( $eventsInformation as $eventIndex => $eventInfo ){
                        ...
                        $insertedCount[$eventInfo->revision] = $eventInfo->created;
                        $updatedCount[$eventInfo->revision]  = $eventInfo->updated;
                        $skippedCount[$eventInfo->revision]  = $eventInfo->skipped;
