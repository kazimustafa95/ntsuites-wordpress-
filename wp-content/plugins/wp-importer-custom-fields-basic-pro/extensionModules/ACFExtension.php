...
class ACFExtension extends ExtensionHandler{
	...
	*/
	public function ACFFields($import_type ,$group) {	
		...
		$get_acf_groups = $wpdb->get_results( $wpdb->prepare("select ID, post_content from $wpdb->posts where post_status != 'trash' and post_type = %s", 'acf-field-group'));
		...
		if ( !empty($group_id_arr) ) {
			foreach($group_id_arr as $groupId) {
				//$get_acf_fields = $wpdb->get_results( $wpdb->prepare( "SELECT ID, post_title, post_content, post_excerpt, post_name FROM $wpdb->posts where post_status != %s and post_parent in (%s)", 'trash',array($groupId) ) );			
				...
				if ( ! empty( $get_acf_fields ) ) {
					...
					foreach ( $get_acf_fields as $acf_pro_fields ) {
						...
						if ( $get_field_content['type'] == 'group' ) {		
							...
							//$get_sub_fields = $wpdb->get_results( $wpdb->prepare( "SELECT ID, post_title, post_content, post_excerpt, post_name FROM $wpdb->posts where post_status != %s and post_parent in (%s)", 'trash',array($group_field) ) );
				...
				if ( ! empty( $group_field_arr ) ) {
					$query1 = "SELECT ID, post_title, post_content, post_excerpt, post_name FROM $wpdb->posts where post_status != 'trash' and post_parent in ($placeholdersForGroupFields)";
