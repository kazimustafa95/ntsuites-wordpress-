...
class ValidateFile {
    ...
    public function file_validation($file_path , $file_extension){
            ...
            if(function_exists('mb_check_encoding') && $file_extension == 'csv') {
    ...
    public function validateCSV($file_path='', $delimiter=','){
        ...
        if(!file_exists($file_path) || !is_readable($file_path))
            ...
            if (($handle = fopen($file_path, 'r')) !== FALSE)
            {
                ...
                if(isset($data[0]) && array_key_exists(null,$data[0])){
                    $valid = 'No';
                }else{
                    $valid = 'Yes';
                }
    ...
    public function import_record_function($hashkey , $filename){
        ...
        $other_taxo = array('nav_menu','action-group','product_type','product_visibility','product_shipping_class','wpsc-variation','wpsc_log_type','post_format','link_category', 'product-category', 'product_cat', 'product_tag');
