...
class ACFProExtension extends ExtensionHandler{
	...
	*/
			...
			foreach($group_id_arr as $groupId) {	
					...
					foreach ( $get_acf_repeater_fields as $acf_pro_repeater_fields ) {						
						...
						if($get_sub_field_content['type'] == 'repeater') {
							...
							if(($get_sub_field_content['type'] !== 'repeater')&&($get_sub_field_content['type'] !== 'group')){	
								...
								$customFields["RF"][ $acf_pro_repeater_fields->post_excerpt ]['label'] = $acf_pro_repeater_fields->post_title;
								//changed
								$customFields["RF"][ $acf_pro_repeater_fields->post_excerpt ]['id']  = $acf_pro_repeater_fields->post_excerpt;
								$customFields["RF"][ $acf_pro_repeater_fields->post_excerpt ]['name']  = $acf_pro_repeater_fields->post_name;
							}
						}
					}
				}
					...
					foreach ( $get_acf_group_fields as $acf_pro_group_fields ) {
						$get_sub_field_content = unserialize( $acf_pro_group_fields->post_content );
							...
							if(($get_sub_field_content['type'] !== 'group')&&($get_sub_field_content['type'] !== 'repeater')){
								// $customFields["GF"][ $acf_pro_group_fields->post_excerpt ]['label'] = $acf_pro_group_fields->post_title;
								// $customFields["GF"][ $acf_pro_group_fields->post_excerpt ]['name']  = $acf_pro_group_fields->post_excerpt;
								$customFields["GF"][  $acf_pro_group_fields->post_excerpt ]['label'] = $acf_pro_group_fields->post_title;
								//changed
								$customFields["GF"][  $acf_pro_group_fields->post_excerpt ]['id']  = $acf_pro_group_fields->post_excerpt;
								$customFields["GF"][  $acf_pro_group_fields->post_excerpt]['name']  = $acf_pro_group_fields->post_name;
							}
					}
				}
				...
				/*if ( ! empty( $get_acf_flexible_content_fields ) ) {
					foreach ( $get_acf_flexible_content_fields as $acf_pro_fc_fields ) {
						...
						$customFields["FC"][ $acf_pro_fc_fields->post_excerpt ]['label'] = $acf_pro_fc_fields->post_title;
						$customFields["FC"][ $acf_pro_fc_fields->post_excerpt ]['name']  = $acf_pro_fc_fields->post_excerpt;
						...
						if($get_sub_field_content['type'] == 'flexible_content'){
							...
							foreach($inner_flexi_query as $inner_flexi_values){
								$customFields["FC"][ $inner_flexi_values->post_excerpt ]['label'] = $inner_flexi_values->post_title;
								$customFields["FC"][ $inner_flexi_values->post_excerpt ]['name']  = $inner_flexi_values->post_excerpt;
							}
						}
					}
				...
				if ( ! empty( $get_acf_flexible_content_fields ) ) {
					foreach ( $get_acf_flexible_content_fields as $acf_pro_fc_fields ) {
						...
						if($get_sub_field_content['type'] !== 'flexible_content'){
							...
							$customFields["FC"][ $acf_pro_fc_fields->post_name ]['label'] = $acf_pro_fc_fields->post_title;
							//changed
							$customFields["FC"][ $acf_pro_fc_fields->post_name ]['id']  = $acf_pro_fc_fields->post_excerpt;
							$customFields["FC"][ $acf_pro_fc_fields->post_name ]['name']  = $acf_pro_fc_fields->post_name;
						}
						...
						if($get_sub_field_content['type'] == 'flexible_content'){
							...
							foreach($inner_flexi_query as $inner_flexi_values){
								...
								$customFields["FC"][ $inner_flexi_values->post_name ]['label'] = $inner_flexi_values->post_title;
								//changed
								$customFields["FC"][ $inner_flexi_values->post_name ]['id']  = $inner_flexi_values->post_excerpt;
								$customFields["FC"][ $inner_flexi_values->post_name ]['name']  = $inner_flexi_values->post_name;
							}
						}
					}
					...
					foreach($flexible_fields as $flexible_id){
						...
						foreach($get_flexible_group as $get_flexible_value){
							foreach($customFields["FC"] as $flexible_key => $flexible_value){
								// $customFields["FC"][$flexible_key]['label'] = $get_flexible_value->post_title;				
								// $customFields["FC"][$flexible_key]['name']  = $get_flexible_value->post_excerpt;
								$customFields["FC"][$get_flexible_value->post_excerpt]['label'] = $get_flexible_value->post_title;
							    //changed
								$customFields["FC"][$get_flexible_value->post_excerpt]['id']  = $get_flexible_value->post_excerpt;
								$customFields["FC"][$get_flexible_value->post_excerpt]['name']  = $get_flexible_value->post_name;
							}
						}	
					}
				}
	...
	public function fetchACFProRepeaterFields($repeater_field) {
		...
		$repeater_field_arr = '';
		$get_sub_fields = $wpdb->get_results( $wpdb->prepare( "SELECT ID, post_title, post_content, post_excerpt, post_name FROM $wpdb->posts where post_status != 'trash' and post_parent in (%s)", array($repeater_field) ) );
		foreach ( $get_sub_fields as $get_sub_key ) {
			$get_sub_field_content = unserialize( $get_sub_key->post_content );
			if ( $get_sub_field_content['type'] == 'repeater' || $get_sub_field_content['type'] == 'group') {
				$repeater_field_arr .= $get_sub_key->ID . ",";
	...
	public function extensionSupportedImportType($import_type){
		...
		if(is_plugin_active('advanced-custom-fields-pro/acf.php')){
			...
			if($import_type =='Posts' || $import_type =='Pages' || $import_type =='CustomPosts' || $import_type =='Users' || $import_type =='Taxonomies' || $import_type =='Tags' || $import_type =='Categories') {		
				return true;
			}
			else{
				return false;
