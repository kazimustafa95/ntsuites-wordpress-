...
class ToolsetImport {
	...
	function set_toolset_values($header_array ,$value_array , $map, $maps, $post_id , $type , $mode,$line_number,$hash_key,$gmode,$templatekey){
		$post_values = [];
		$helpers_instance = ImportHelpers::getInstance();
		$post_values = $helpers_instance->get_header_values($map , $header_array , $value_array);
		$post_val = $helpers_instance->get_meta_values($maps , $header_array , $value_array);
		$this->types_import_function($post_values,$post_val,$type, $post_id, $mode,$line_number,$hash_key,$gmode,$templatekey);
	}

	public function types_import_function($data_array,$post_val, $type, $postId ,$mode,$line_number,$hash_key,$gmode,$templatekey) 
	{
		$wptypesfields=array();
		...
		if ($mode=='Update' || $mode == 'Import-Update') {
			...
			if(!empty($data_array)) {
				...
				$WPToolsetUpdater->update($postId,$post_val,$line_number,$hash_key,$gmode,$templatekey);
			}
		}elseif ($mode =='Insert') {
			...
			if(!empty($data_array)) { 
				...
				$wpToolsetImporter->import($postId,$post_val,$line_number,$hash_key,$gmode,$templatekey);
			} 
		}
	...
	{  
		...
		$wpdb->delete($wpdb->prefix.'toolset_associations',array('child_id' => $pID));
	...
	public function intermediateRelationship($data_array,$pID)
	{
		...
		$wpdb->delete($wpdb->prefix.'toolset_associations',array('intermediary_id' => $pID));
