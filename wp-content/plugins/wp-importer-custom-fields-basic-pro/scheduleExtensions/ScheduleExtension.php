...
class ScheduleExtension {
	...
	private static $save_mapping = null,$ftp_upload,$schedule_import = null,$smack_instance,  $url_upload , $sftp_upload = null;
	...
	public function saveEventInformationToSchedule() {
		...
		$import_method = sanitize_text_field($_POST['method']);
		$time_zone = $_POST['UTC'];
		if(empty($time_zone)){
		...
		$duplicateHeaders = sanitize_text_field($_POST['duplicate_header']);

		if(isset($_POST['UpdateUsing'])){
			update_option('custom_fields_importer_update_using', sanitize_text_field($_POST['UpdateUsing']));
		}
		...
		$timestamp = strtotime(sanitize_text_field($_POST['date']));
		$dbdate = date("Y-m-d", $timestamp);
		$wpdb->insert('ultimate_cfimporter_pro_scheduled_import',
		...
		if(!is_wp_error($wpdb->insert_id)) {
			$wpdb->update($schedule_tableName, ['hook_name' => 'smack_cf_cron_schedule_function_'. $wpdb->insert_id], ['id' => $wpdb->insert_id]);

			if ($import_method == 'url') {
				$save_externalfile = "insert into ultimate_cfimporter_pro_external_file_schedules (schedule_id, file_url, filename) values ('{$wpdb->insert_id}', '".sanitize_text_field($_POST['fileurl'])."', '$eventKey')";
				$wpdb->query($save_externalfile);

			} else {
				if ($import_method == 'ftp' || $import_method == 'ftps' || $import_method == 'sftp') {
					$save_ftpfile = "insert into ultimate_cfimporter_pro_ftp_schedules (schedule_id, hostname, username, password, initial_path, filename, port_no) values ('{$wpdb->insert_id}', '".sanitize_text_field($_POST['hostname'])."', '".sanitize_text_field($_POST['username'])."', '".sanitize_text_field($_POST['password'])."', '".sanitize_text_field($_POST['initialpath'])."', '$eventKey' , '".sanitize_text_field($_POST['portnumber'])."')";
	...
	public  function smack_uci_cron_scheduler() {		
		global $wpdb;
		$endDate = '';
		$exturl_schedule_table = 'ultimate_cfimporter_pro_external_file_schedules';
		$schedule_tableName = 'ultimate_cfimporter_pro_scheduled_import';
		...
		if(!empty($timeZone)){
			...
			$scheduleList = $wpdb->get_results("SELECT * FROM ultimate_cfimporter_pro_scheduled_import WHERE isrun = 0 AND nexrun <= '$current_timestamp' AND cron_status != 'completed' ");
		}
		if (!empty($scheduleList)) {
			foreach ($scheduleList as $scheduledEvent) {
				$wpdb->query("update ultimate_cfimporter_pro_scheduled_import set isrun = 1 where id = '{$scheduledEvent->id}'");
				$runSchedule = false;
				$data = array();
				...
				$hashkey = $wpdb->get_results("select eventkey from ultimate_cfimporter_pro_mappingtemplate where id = $templateid");
				...
				if ($runSchedule === true && $scheduledEvent->importbymethod == 'url') {
					$external_scheduleList = $wpdb->get_results("select filename, file_url from ultimate_cfimporter_pro_external_file_schedules where schedule_id = $scheduledEvent->id");
					if ($external_scheduleList[0]->filename != '') {
						...
						if (isset($returnData['Failure'])) {
							...
							$template_info = $wpdb->get_results("select mapping , module from ultimate_cfimporter_pro_mappingtemplate where eventKey = '{$data1['eventkey']}'",ARRAY_A);
							$save_template = "insert into ultimate_cfimporter_pro_mappingtemplate (mapping ,module,eventKey) values('{$template_info[0]['mapping']}','{$template_info[0]['module']}' , '{$data['csv_name']}' )";
				...
				elseif ($runSchedule === true && $scheduledEvent->importbymethod == 'ftp') {
					$ftp_scheduleList = $wpdb->get_results("select * from ultimate_cfimporter_pro_ftp_schedules where schedule_id = $scheduledEvent->id");
					...
					if ($ftp_scheduleList[0]->filename != '') {
						...
						// $returnData = $ftpupload->ftp_upload($data);
						...
						if (isset($returnData['Failure'])) {
							...
							$template_info = $wpdb->get_results("select mapping , module from ultimate_cfimporter_pro_mappingtemplate where eventKey = '{$data1['eventkey']}'",ARRAY_A);
							$update_mapping = $template_info[0]['mapping'];
					
							$wpdb->query("UPDATE ultimate_cfimporter_pro_mappingtemplate SET mapping = '$update_mapping' WHERE event_key = '".$data['eventkey']."'",ARRAY_A);
							$save_template = "insert into ultimate_cfimporter_pro_mappingtemplate (mapping ,module,eventKey) values('{$template_info[0]['mapping']}','{$template_info[0]['module']}' , '{$data['csv_name']}' )";
							$wpdb->query($save_template);
							$wpdb->query("UPDATE $file_table_name SET hash_key = '".$returnData['hashkey']."' WHERE hash_key = '".$data['eventkey']."'");
							$data['eventkey'] = $returnData['hashkey'];
				...
				elseif ($runSchedule === true && $scheduledEvent->importbymethod == 'sftp') {
					...
					if ($ftp_scheduleList[0]->filename != '') {
						...
						if (isset($returnData['Failure'])) {
							...
							$data['uploaded_name'] = $returnData['filename'];
							$wpdb->query("update {$ftp_schedule_table} set filename = '{$returnData['hashkey']}' where schedule_id = '{$scheduledEvent->id}'");
							$template_info = $wpdb->get_results("select mapping , module from ultimate_cfimporter_pro_mappingtemplate where eventKey = '{$data['eventkey']}'",ARRAY_A);
							$save_template = "insert into ultimate_cfimporter_pro_mappingtemplate (mapping ,module,eventKey) values('{$template_info[0]['mapping']}','{$template_info[0]['module']}' , '{$data['csv_name']}' )";
							$wpdb->query($save_template);
							$wpdb->query("UPDATE $file_table_name SET hash_key = '".$returnData['hashkey']."' WHERE hash_key = '".$data['eventkey']."'");
							$data['eventkey'] = $returnData['hashkey'];
	...
	public static function getTemplateInformation($eventKey) {
		global $wpdb;
		$templateid = $wpdb->get_col($wpdb->prepare("select id from ultimate_cfimporter_pro_mappingtemplate where eventKey = %s",$eventKey));
	...
	public  function doSchedule($data,$current_timestamp) {
		...
		if ($getScheduling_data[0]->cron_status == 'initialized') {
			...
			$wpdb->query("update ultimate_cfimporter_pro_scheduled_import set cron_status = 'initialized' where id = '{$data['id']}'");
		}
		...
		//if($data['frequency'] != 0) {
			$wpdb->query("UPDATE ultimate_cfimporter_pro_scheduled_import SET isrun = 0 WHERE id = $data_id");
		//}
		$check_schedule_complete = $wpdb->get_results("SELECT id FROM ultimate_cfimporter_pro_scheduled_import WHERE cron_status = 'completed' AND isrun = 1 AND id = $data_id");
