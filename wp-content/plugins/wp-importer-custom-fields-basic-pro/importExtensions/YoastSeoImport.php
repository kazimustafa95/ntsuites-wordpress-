...
class YoastSeoImport {
    ...
    function set_yoast_values($header_array ,$value_array , $map, $post_id , $type,$hash_key,$gmode,$templatekey){	
		...
		$this->yoast_import_function($post_values,$type, $post_id,$hash_key,$gmode,$templatekey);
    }

    function yoast_import_function($data_array, $importas, $pID,$hash_key,$gmode,$templatekey) {
		...
		foreach (get_taxonomies() as $item => $taxonomy_name) {
			if($taxonomy_name == $importas) {
				...
				if(isset($data_array['opengraph-image'])) {
					...
					$image_id = $media_instance->image_meta_table_entry('', $pID, 'opengraph-image', $data_array['opengraph-image'], $hash_key, 'yoast_opengraph', 'post',$templatekey,$gmode);
				...
				if(isset($data_array['twitter-image'])) {
					...
					$imageid = $media_instance->image_meta_table_entry('', $pID, 'twitter-image', $data_array['twitter-image'], $hash_key, 'yoast_twitter', 'post',$templatekey,$gmode);
