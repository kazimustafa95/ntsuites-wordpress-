...
class WPToolsetUpdater extends ToolsetImporter
{
	...
	function update($postId,$post_val,$line_number,$hash_key,$gmode,$templatekey) {
		...
		if (!empty($allRelationship)) {
			...
			foreach ($allRelationship as $key => $value) {
				...
				if($plugin_version < '3.4.1'){
					...
					$pquery = "select id from {$wpdb->prefix}toolset_connected_elements where element_id ='{$value['id']}'";	
					$pqueryres1 = $wpdb->get_results($pquery);
					$p_id = $pqueryres1[0]->id;
			...
			foreach ($getMetaData as $key => $value) {					
				...
				if (array_key_exists($metaKey, $fieldTypeArray)) {
					...
					if ($fieldType == 'post') {
						...
						$this->checkFieldType($metaId,$fieldType,$metaKey,$line_number);
					}else{
	...
	function checkFieldType($postId,$fieldType,$metaKey,$line_number){
